...
class Membership extends Component {
  constructor() {
    ...
    this.handleMembershipDescription = this.handleMembershipDescription.bind(
      this
    );
  ...
  updateMemb() {
    ...
    axios
      .put("/api/membershipdetails/" + this.state.editing, body)
      .then(res => {
        this.setState({
          editing: null,
          membEditModal: false,
            ...
            if (e.available) {
              ...
              return (
                <div key={i} className="membershiptile">
                  <h2 className="membershiptitle">{e.membership_title}</h2>
                  <div className="membershipdesc">{e.membership_desc}</div>
                  <div className="adminbuttoncontainer">
                    {!this.props.user.is_admin ? null : (
                      <button
                        className="adminbutton"
                        onClick={() => this.membEditModal(e.membership_id)}
                      >
                        Edit
                      </button>
                    )}
                    {!this.props.user.is_admin ? null : (
                      <button
                        className="adminbutton"
                        onClick={() => this.membDeleteModal(e.membership_id)}
                      >
                        Delete
                      </button>
                    )}
              ...
              <div key={i} className="selectfield">
                ...
                <div>
                  <input
                    checked={e.available}
                    onChange={() =>
                      this.makeAvailable(e.membership_id, e.available)
                    }
                    type="checkbox"
                  />
                </div>
    ...
    return (
      <div>
        <div className="membershipbody">
          ...
          <div className="membershipcontainer">{membershipMap}</div>
        ...
        <Modal
          isOpen={this.state.addMembershipModal}
          onRequestClose={this.closeAddMembershipModal}
          style={addStyle}
        >
          ...
          <div className="checkboxfield">
            ...
            <input
              ...
              value={this.state.membershipTitle}
              onChange={e => this.handleMembershipTitle(e.target.value)}
              type="text"
            />
          </div>
          <div className="checkboxfield">
            ...
            <input
              ...
              value={this.state.membershipDescription}
              onChange={e => this.handleMembershipDescription(e.target.value)}
              type="text"
            />
          </div>
          <div className="checkboxfield">
            ...
            <input
              ...
              onChange={e => this.handleMembershipPrice(e.target.value)}
              type="text"
            />
          </div>
          <div className="checkboxfield">
            Is this Recurring?{" "}
            <input
              value={this.state.membershipRecurring}
              onChange={e => this.handleMembershipRecurring(e.target.checked)}
              type="checkbox"
            />
          </div>
          <div className="checkboxfield">
            Membership Period:{" "}
            <input
              placeholder="If lifetime, leave blank"
              value={this.state.membershipPeriod}
              onChange={e => this.handleMembershipPeriod(e.target.value)}
              type="text"
            />
          </div>
        ...
        <Modal
          isOpen={this.state.selectAvailable}
          onRequestClose={this.closeSelectAvailable}
          style={addStyle}
        >
        ...
        <Modal
          isOpen={this.state.membEditModal}
          onRequestClose={this.closeMembEditModal}
          style={addStyle}
        >
          <button onClick={this.closeMembEditModal}>Close</button>
          <div className="checkboxfield">
            You are only able to update the title and description and this only
            effects the db and how it is displayed on the page. If you want to
            change period, recurring, or cost that needs to be done with the add
        ...
        <Modal
          isOpen={this.state.membDeleteModal}
          onRequestClose={this.closeMembDeleteModal}
          style={addStyle}
        >
          <button onClick={this.closeMembDeleteModal}>Close</button>
          <div className="checkboxfield">
            This will only delete from the database and available memberships
            and not from stripe. Are you sure you want to delete?
          </div>
