...
class SingleBook extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .all([book, savedbooks, practiceProblems, completedProblems])
      .then(res => {
        this.setState({
          book: res[0].data,
  ...
  addToSaved() {
    axios
      .post("/api/savedbooks", { book_id: this.state.book[0].book_id })
      .then(books => {
        this.setState({
  ...
  removeFromSaved() {
    axios
      .delete("/api/savedbooks/" + this.state.book[0].book_id)
      .then(books => {
        this.setState({
          savedbooks: books.data
        });
  ...
  render() {
    let adminaddChapter = !this.props.user.is_admin ? null : (
      <button className="adminbutton" onClick={() => this.openAddModal()}>
        Add Chapter
      </button>
    );
    let chapters =
      this.state.book.length > 0 ? this.state.book[0].chapters : null;
      ...
      ? chapters.map((e, i) => {
          return (
            <div className="chaptercontainer" key={i}>
              <Link
                to={`/book/${this.props.match.params.book}/${e.chapter_id}`}
                key={i}
              ...
              <div className="adminbuttoncontainer">
                {!this.props.user.is_admin ? null : (
                  <button
                    ...
                    onClick={() => this.openChapEditModal(e.chapter_id)}
                  >
                    Edit
                  </button>
                )}
                {!this.props.user.is_admin ? null : (
                  <button
                    className="adminbutton"
                    onClick={() => this.openChapDeleteModal(e.chapter_id)}
                  >
                    Delete
                  </button>
                )}
    ...
    let bookImg =
      this.state.book.length > 0
        ...
        : null;
    let bookTitle =
      this.state.book.length > 0 ? this.state.book[0].book_title : null;
    let bookSubTitle =
      this.state.book.length > 0 ? this.state.book[0].book_subtitle : null;
    let bookAuthor =
      this.state.book.length > 0
        ? this.state.book[0].author.map((e, i) => {
    ...
    let addRemoveButton = !this.props.user.user_id ? null : savedBooksExist ? (
        ...
        : this.state.practiceProblems.map((e, i) => {
            ...
            let completedTotal =
              typeof completed === "object" && completed.length > 0
                ...
                : 0;
            let tileClass =
              completedTotal === e.practiceProblems.length
                ? "practicedone"
                : "practicetile";
            return (
              <div className={tileClass} key={e.sectionTitle}>
                <Link to={`/practice/${this.props.match.params.book}/${e.chapter}/${e.sectionNumber}`}>
                  <div className="practicetilehead">
                    {e.chapter}.{e.sectionNumber}
                  </div>
      ...
      <div className="singlebookbody">
          ...
          <div className="booksummary">
            ...
            <div className="adminbuttoncontainer">
              {!this.props.user.is_admin ? null : (
                <button
                  className="adminbutton"
                  onClick={() => this.openEditModal(this.state.book[0].book_id)}
                >
                  Edit
                </button>
              )}
              {!this.props.user.is_admin ? null : (
                <button
                  className="adminbutton"
                  onClick={() =>
                    this.openDeleteModal(this.state.book[0].book_id)
                  }
                >
                  Delete
                </button>
        ...
        <div className="singlebookcontent">
          <div className="chapterlist">
            <h1>STUDY</h1>
            {adminaddChapter}
            <div className="chaptergrid">{chapterList}</div>
          </div>
          <div className="practicelist">
            <h1>PRACTICE</h1>
            <div className="practicegrid">{practiceMap}</div>
          </div>
        </div>
        ...
        >
          ...
          <div className="checkboxfield">
            ...
            <input
              ...
              onChange={e => this.handleChapTitle(e.target.value)}
            />
          </div>
          <div className="checkboxfield">
            Chapter Number:{" "}
            <input
              ...
              value={this.state.bookChapter}
              onChange={e => this.handleBookChap(e.target.value)}
            />
          </div>
          <div className="checkboxfield">
            Chapter Summary:{" "}
            <input
              ...
              onChange={e => this.handleChapText(e.target.value)}
            />
          </div>
          <div className="checkboxfield">
            ...
            <input
              type="checkbox"
              defaultChecked={this.state.chapterMemRequired}
              onChange={e => this.handleChapMemRequired(e.target.checked)}
            />
          </div>
          <div className="checkboxfield">
            Membership IDs:{" "}
            <input
              ...
              value={this.state.chapterMemIds}
              onChange={e => this.handleChapMemIds(e.target.value)}
            />
          </div>
        ...
        <Modal
          isOpen={this.state.chapEditModal}
          onRequestClose={this.closeChapEditModal}
          style={addStyles}
        >
        ...
        <Modal
          isOpen={this.state.chapDeleteModal}
          onRequestClose={this.closeChapDeleteModal}
          style={addStyles}
        >
